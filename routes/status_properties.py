...
def status(vv_instance):
    ...
    return jsonify(success_response({"result": result}, "VibrationVIEW status retrieved successfully"))
...
def is_ready(vv_instance):
    ...
    return jsonify(
        success_response({"result": result}, f"VibrationVIEW hardware is {'ready' if result else 'not ready'}")
    )
...
def is_running(vv_instance):
    ...
    return jsonify(success_response({"result": result}, f"Test is {'running' if result else 'not running'}"))
...
def is_starting(vv_instance):
    ...
    return jsonify(success_response({"result": result}, f"Test is {'starting' if result else 'not in startup phase'}"))
...
def is_changing_level(vv_instance):
    ...
    return jsonify(
        success_response({"result": result}, f"Test is {'changing levels' if result else 'not changing levels'}")
    )
...
def is_hold_level(vv_instance):
    ...
    return jsonify(success_response({"result": result}, f"Test is {'on hold' if result else 'not on hold'}"))
...
def is_open_loop(vv_instance):
    ...
    return jsonify(
        success_response({"result": result}, f"Test is {'in open loop mode' if result else 'in closed loop mode'}")
    )
...
def is_aborted(vv_instance):
    ...
    return jsonify(
        success_response({"result": result}, f"Test {'has been aborted' if result else 'has not been aborted'}")
    )
