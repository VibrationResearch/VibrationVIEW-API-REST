    ...
    docs = {
        ...
        'endpoints': {
            ...
            'POST /runtest': {
                ...
                'com_method': 'RunTest(filepath) or OpenTest() + StartTest()',
                'parameters': {
                    'testname': 'string - Query parameter with VibrationVIEW profile filename'
                },
                'returns': 'boolean - Test running status verified by IsRunning()',
                'example': 'POST /api/runtest?testname=test1.vsp'
            },
            'PUT /runtest': {
                'description': 'Upload and run a complete test (combines upload + OpenTest + StartTest)',
                'com_method': 'OpenTest(filepath) + StartTest()',
                'parameters': {
                    'filename': 'string - Query parameter with desired filename',
                ...
                'example': 'PUT /api/runtest?filename=test1.vsp (with binary file in body)'
            },
           'POST /stoptest': {
            ...
            'POST /opentest': {
                ...
                'example': 'POST /api/opentest?testname=test1.vsp'
            },
            'PUT /opentest': {
                ...
                'example': 'PUT /api/opentest?filename=test1.vsp (with binary file in body)'
            }
        },
        'notes': [
...
@basic_control_bp.route('/runtest', methods=['POST'])
@handle_errors
@with_vibrationview
def run_test(vv_instance):
    ...
    Query Parameters:
        testname: VibrationVIEW profile filename
    
    Example: POST /api/runtest?testname=test1.vsp
    """
    # Get test name from query string
    test_name = request.args.get('testname')
    
    if not test_name:
        return jsonify(error_response(
            'Missing required query parameter: testname',
            'MISSING_PARAMETER'
        )), 400
...
@basic_control_bp.route('/opentest', methods=['POST'])
@handle_errors
@with_vibrationview
