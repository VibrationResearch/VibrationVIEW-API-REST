...
class TestWriteGuardBlocks:
    ...
    def test_get_blocked(self, guarded_client, endpoint):
        ...
        assert response.status_code == 405, f"GET /api/v1/{endpoint} should be 405, got {response.status_code}"
    ...
    def test_post_allowed(self, guarded_client, endpoint):
        ...
        assert response.status_code != 405, f"POST /api/v1/{endpoint} should not be 405, got {response.status_code}"
        ...
        for rule in sorted(guarded_app.url_map.iter_rules(), key=lambda r: r.rule):
            if "GET" in rule.methods and rule.rule.startswith(prefix):
                if rule.rule not in blocked:
                    ...
                    assert response.status_code != 405, f"GET {rule.rule} should not be 405, got {response.status_code}"
