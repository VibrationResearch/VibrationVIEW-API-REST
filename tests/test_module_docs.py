...
class TestModuleDocumentation:
    ...
    def test_module_docs_has_endpoints(self, client, module_name):
        ...
        assert "endpoints" in data, f"Module '{module_name}' docs missing 'endpoints' section"
        assert isinstance(data["endpoints"], dict), f"Module '{module_name}' endpoints should be a dictionary"
        assert len(data["endpoints"]) > 0, f"Module '{module_name}' has no documented endpoints"
    ...
    def test_module_docs_has_module_name(self, client, module_name):
        ...
        assert "module" in data, f"Module '{module_name}' docs missing 'module' field"
        assert data["module"] == module_name, f"Module '{module_name}' has wrong module name: {data.get('module')}"
...
class TestDocumentationContent:
    ...
    def test_endpoint_docs_have_description(self, client, module_name):
        ...
        if "endpoints" in data:
            ...
            assert len(endpoints) > 0, f"Module '{module_name}' has no endpoint definitions"
    ...
    def test_all_modules_accessible(self, client):
        ...
        assert len(failed_modules) == 0, f"The following modules are missing docs endpoints: {failed_modules}"
