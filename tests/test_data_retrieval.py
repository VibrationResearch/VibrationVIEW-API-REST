...
class TestDataRetrieval:
    ...
    def test_debug_available_routes(self, client):
        ...
        with client.application.app_context():
            ...
            for rule in current_app.url_map.iter_rules():
                ...
                if "/api/v1/" in rule.rule and any(
                    endpoint in rule.rule
                    for endpoint in ["demand", "control", "channel", "output", "vector", "channelunit", "controllabel"]
                ):
    ...
    def test_debug_singleton_behavior(self, client):
        ...
        print(f"Current singleton instance: {_singleton._vv_instance}")
        ...
        for endpoint, loopnum in test_cases:
            ...
            if response.status_code != 404:
                ...
                assert "must be >= 1" in data["error"]["message"], (
                    f"{endpoint} error message missing 'must be >= 1' for loopnum={loopnum}"
                )
        ...
        for endpoint, expected_error in invalid_tests:
            ...
            if response.status_code != 404:
                ...
                assert expected_error in data["error"]["message"], (
                    f"{endpoint} error message missing '{expected_error}'"
                )
